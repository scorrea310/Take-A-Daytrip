    ...
    const onSave = () => {
        ...
        if (label === "Name") {
            if (inputValue.length === 0 || inputValue.length > 255) {
                setErrors("Name must be greater that 0 and less than or equal to 255 characters.")
                return;
            }
            ...
            if (inputValue.length === 0 || inputValue.length > 40) {
                setErrors("Username must be greater that 0 and less than or equal to 40 characters.")
                return;
            }
            setLoadingChange(true)
            ...
            if (inputValue.length === 0 || inputValue.length > 40) {
                setErrors("Email must be greater that 0 and less than or equal to 255 characters.")
                return;
            } else {
            ...
            {editing &&
                <div className="label-and-initial-value-container">
                    ...
                    <div className="saveButtonAndErrorsContainer">
                        <div onClick={onSave} className="save-button-personal-info">{loadingChange ? <FadingBalls color="white" width="10px" height="10px" /> : "Save"}</div>
