...
const ReservedSpotCard = ({ reservation }) => {
  ...
  const [priceState, setPriceState] = useState(
    (days * reservation.price).toFixed(2)
  );
      ...
      <div className="reservesSpotCardInfo">
        <div className="yourGoingToTextAndEditContainer">
          ...
          <div className="editAndDeleteReservationIconAndText">
            <div
              className="editIconAndDeleteReserveCard"
              onClick={() => setShowEditModal(true)}
            >
              <FiEdit id="editReservationIcon" /> Edit{" "}
            </div>
            <div
              className="deleteIconAndTextReserveCard"
              onClick={() => setCancelReservation(true)}
            >
              <MdOutlineCancelPresentation id="cancelReservationIcon" /> Cancel
            </div>
            {showEditModal && (
              <Modal
                idName={"edit-Reservation-Modal"}
                onClose={() => setShowEditModal(false)}
              >
                <ReserveSpot
                  editStartDate={reservation.check_in}
                  editEndDate={reservation.check_out}
                  reservationDate={reservation.reservation}
                  setShowEditModal={setShowEditModal}
                  totalOccupantsAllowed={reservation.total_occupants_allowed}
            ...
            {showCancelReservationModal && (
              <Modal
                idName={"edit-Reservation-Modal"}
                onClose={() => setCancelReservation(false)}
              >
                <div className="cancelReservationInModalContainer">
                  ...
                  <div className="cancelReservationInModalButtonContainer">
                    ...
                    />
                    <Button
                      className={"noCancelReservationButton"}
                      label={"No"}
