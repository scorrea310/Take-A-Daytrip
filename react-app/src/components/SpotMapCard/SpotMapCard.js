...
const SpotMapCard = ({ setFocusedSpotMark, spot }) => {
  ...
  const handlePageChange = (e) => {
    if (
      e.target.className.baseVal === "spotMapCardNextArrow" ||
      e.target.nodeName === "path" ||
      e.target.id === "closeMapBoxIcon"
    ) {
      return;
    }
  ...
  return (
    <div
      id="spotMapCard"
      onMouseOver={() => setImageArrowsVisible(true)}
      onMouseLeave={() => setImageArrowsVisible(false)}
    ...
    >
      <div
        style={{
          width: "100%",
          height: "100%",
          backgroundImage: `url(${spot.images[currentImageDisplayed]})`,
      ...
      >
        <div id="spotMapCardCloseAndArrowsMainContent">
          <IoIosClose
            id="closeMapBoxIcon"
            onClick={() => setFocusedSpotMark(false)}
          />
              ...
              <div
                id={
                  currentImageDisplayed === 0
                    ? "spotMapCardArrowsMainContainerRight"
                    : "spotMapCardArrowsMainContainerLeft"
                }
              >
                {currentImageDisplayed !== 0 && (
                  <AiOutlineLeft
                    className="spotMapCardNextArrow"
                    onClick={() => setCurrentImageDisplayed(0)}
                  />
                )}
                {currentImageDisplayed === 0 && (
                  <AiOutlineRight
                    className="spotMapCardNextArrow"
                    onClick={() => setCurrentImageDisplayed(1)}
                  />
                )}
              </div>
            )}
